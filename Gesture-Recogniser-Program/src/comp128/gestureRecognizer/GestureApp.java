...
public class GestureApp {
    ...
    private void setupUI(){
        ...
        canvas.onDrag(event -> {
            Line line= new Line(path.peekLast(), event.getPosition());
            path.addLast(event.getPosition());
            canvas.add(line);
        });

        canvas.onMouseUp(event -> {
            if (!(recognizer.isEmpty())){
                Result result=recognizer.recognize(path);
            double score = result.getScore();
            matchLabel.setText( "Match: "+ result.getTemplate().getName()+ score);
            }
